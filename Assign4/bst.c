...
/*
 ...
 Name: Eli Goodwin
 Date: 2017.02.15
 Solution description: Creates and tests a simple binary search tree container without autobalancing.
...
{
	/* Write this */
    //if cur == null return new node with value parameter
    if(cur == NULL){
    ...
    // if val is less than current node value go left or right and continuing doing so until the correct position is found
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    struct Node* traverseNode = tree->root; //node used for traversal of the tree
    int result = 0; //stores the result for determining which branch of root to traverse
    int found = 0; //flag for if the value was found
    
    //while the next node is not equal to void and the search value is not found compare and traverse
    while(traverseNode != NULL && found != 1){
        result = compare(traverseNode->val, val);
...
{
    //recursive left most traversal
    assert(cur != NULL);
    //if the left most branch is null, stop and return value
    if(cur->left == NULL){
        return cur->val;
    }
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    /*write this*/
    assert(cur != NULL);
    //compare val to the current node value
    int result = compare(val, cur->val);
    
    //if the cur node and passed value are equal the value is found and start removal
    if(result == 0){
        if(cur->right == NULL){
            free(cur->val);
            free(cur);
    ...
    //keep searching for the value if it hasn't been found
    else if(result == 1){

...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    //clear if visited
    clearVisited(graph);
    //make the stack for reachables
    struct Deque *reachables = dequeNew();
    //push source on to the stack
    dequePushFront(reachables, source);
    Vertex* currentVertex = NULL;
 
    
    //while the deque of reachables is not empty
    while(!dequeIsEmpty(reachables)){
        //get the first item of the stack
        currentVertex = dequeFront(reachables);
        dequePopFront(reachables);
        //is the item the deistnation, if so return it
        if(currentVertex == destination){
            //destroy the deck
            dequeDelete(reachables);
            return 1;
        }
        //if it is not, push all the reachable nodes onto the stack
        if(currentVertex->isVisited == 0){
            //mark as visited
            currentVertex->isVisited = 1;
            //for those nodes not vistied go through the neighbors list and push on to the stack
            for(int i = 0; i < currentVertex->numNeighbors; ++i){
                if(!currentVertex->neighbors[i]->isVisited){
                    dequePushFront(reachables, currentVertex->neighbors[i]);
                }
            }
        }
    }
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    while(!dequeIsEmpty(reachables)){
        ...
        //add all neighbors to the end of the queue if not visited
        if(currentVertex->isVisited == 0){
            //mark as visited
            currentVertex->isVisited = 1;
            //go through all the neighbors and those that are not visited add them to the back of the queue
            for(int i = 0; i < currentVertex->numNeighbors; ++i){
                if(!currentVertex->neighbors[i]->isVisited){
                    dequePushBack(reachables, currentVertex->neighbors[i]);
                }
